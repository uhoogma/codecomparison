...
public class Similarity {
	...
	private static double calculateJaccardCoefficient(Collection<Integer> collection1, Collection<Integer> collection2,
	...
	public static double JaccardCoefficient(String first, String second, String boilerPlate, int ngramSize,
		...
		Collection<Integer> commonCollection = collection1.stream().filter(item -> collection2.contains(item))
				.collect(Collectors.toList());

		return calculateJaccardCoefficient(collection1, collection2, commonCollection);
	}

	public static double JaccardCoefficientFromHashes(int[] convertedHashesFirst, int[] convertedHashesSecond,
			int windowSize) {
