...
public class RemoveConditionals extends Java8BaseListener {
	 ...
	 * structures, but eliminate boolean expressions. There are many obfuscation
	 * methods that disguise the actual boolean expressions to fail the
	 * text/token based similarity detection tools. In order to lessen the
	 * effect of such obfuscations, we abstract away the expression in the if
	 * statement. With similar view points, if-else statements and switch
