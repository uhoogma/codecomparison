...
public class RemoveLoops extends Java8BaseListener {
	 ...
	 * initialization expression, loop condition expression, and increment
	 * expression. These expressions are quite easily obfuscated by malicious
	 * theft. For example, changing variable names in for expressions can not
	 * affect actual execution of program. Also changing expressions is
	 * available to equal to the number of times the execution of the loop.
	 * Thus, these for expressions are eliminated by generating abstracted
	 * source code. So that only loop constructs can remain keeping their loop
	 * structures, like form for ( ; ; ) . Similarly, while and do-while
	 * statements are abstracted by eliminating the enclosed condition
	 * expressions and only the control structures remain in the abstracted
	 * source code.
	 */
	/*
	 * basicForStatement basicForStatementNoShortIf ;; enhancedForStatement
